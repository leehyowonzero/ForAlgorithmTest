...
def solution(n, m, x, y, queries):
    ...
    for query in reversed(queries):
        ...
        if(d == 1): # 열번호가 증가하는 방향으로 이동하는 명령이므로 현재 유망한 사각형이 n-1번 열에 존재하는지가 중요함, n-1 번열이면 범위를 확장하고 아닐 시 좌표만 당김
            if(upper_y == m-1):
                lower_y -= dxy[d][1] * dx
            else:
                lower_y -= dxy[d][1] * dx
        ...
        if(d == 2): # 행번호가 감소하는 방향으로 이동하는 명령이므로 현재 유망한 사각형이 0번 행에 존재하는지가 중요함, 0번행이면 범위를 확장하고 아닐 시 좌표만 당김
            ...
            if(upper_x >= n):
                upper_x = n-1
        if(d == 3): # 행번호가 증가하는 방향으로 이동하는 명령이므로 현재 유망한 사각형이 m-1번 행에 존재하는지가 중요함, m-1번행이면 범위를 확장하고 아닐 시 좌표만 당김
            if(upper_x == n-1):
                lower_x -= dxy[d][0] * dx
            else:
                lower_x -= dxy[d][0] * dx
        ...
        # print(tmp)
        if(tmp < 0): # 음수가 나온다는 건 범위가 박살이 났다는 것이므로 불가능 한 경우임
            return 0
