...
for item in archers_case: # 모든 궁수 조합에 대해
    result = 0
    copy_board = copy.deepcopy(board) # 궁수 조합에 따라 다시해야하므로 deep copy 한다.
    for archs_row in range(N + 1, 1, -1): # 가장가까운 행부터
        for archs_col in item: # 각 조합의 궁수 위치에 대해
            if shoot_cnt < 3:  # 3발 보다 적게 쐈다면
                if can_shoot(copy_board, archs_row, archs_col, D):
                    shoot_cnt += 1

        shoot_cnt = 0
        for enemy in died: # 쏜 적에 대해
            if copy_board[enemy[0]][enemy[1]] == 1: # 이미 쏘지 않은 적이면
                result += 1 # 죽인 횟수에 더한다
                copy_board[enemy[0]][enemy[1]] = 0
